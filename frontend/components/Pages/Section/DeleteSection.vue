            ...
            <div class="flex justify-center">
                <div class="text-center">
                    ...
                    <div class="flex justify-center mt-8">
                        <nuxt-link class="mx-2 group relative flex justify-center rounded-md border border-transparent bg-gray-600 py-3 px-4 text-sm font-medium text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2" :to="`/${accessType}/sections`">Cancel</nuxt-link>
                        <button @click="deleteSection" type="submit" class="group relative flex justify-center rounded-md border border-transparent bg-red-600 py-3 px-4 text-sm font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
...
<script setup lang="ts">
   ...
   const loading = ref(false);
    ...
    function deleteSection() {
        loading.value =  true;
        section.delete(props.id).then(res => {
            if (res.data.value) {
                success.value = 'Successfully deleted section.';
                setTimeout(() => {
                    router.push(`/${props.accessType}/sectionss`);
                }, 2000);
            }
        });
    }
