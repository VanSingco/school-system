    ...
    <NuxtLayout :name="accessType">
        <div>
            <Header :breadcrumbs="header.breadcrumbs" :title="header.title">
                <nuxt-link :to="`/${props.accessType}/users/add`" class="group relative flex justify-center button-primary">
                            ...
                            <form action="" method="post" @submit.prevent="searchUser">
                                <div class="grid grid-cols-1 gap-4">
                                    <div>
                                        ...
                                        <input v-model="search" type="text" placeholder="Search User"  class="mt-1 py-3 px-3 block w-full rounded-md border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">User Type</label>
                                        <select v-model="user_type" name="" id="" class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-3 px-3 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm">
                            ...
                            <Menu as="div" class="relative ml-3">
                                ...
                                <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                                    <MenuItems class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow ring-1 ring-black ring-opacity-5 focus:outline-none">
                                        <MenuItem>
                                            <nuxt-link :to="`/${props.accessType}/users/edit/${row.id}`" class="block px-4 py-2 text-sm text-gray-500">
                                                Edit User
                                            </nuxt-link>
                                        </MenuItem>
                                        <MenuItem>
                                            <nuxt-link :to="`/${props.accessType}/users/delete/${row.id}`" class="block px-4 py-2 text-sm text-gray-500">
                                                Delete User
                                            </nuxt-link>
                                        </MenuItem>
                                    </MenuItems>
...
<script setup lang="ts">
    ...
    const search = ref('');
    const user_type = ref('super-admin');
    const orderBy = ref('ASC');
    const page = ref(1);

    const user_loading = ref(false);
    const user = useUserStore();
    const columns = ['Name', 'Email', 'Type', 'Actions']
    ...
    function fetchUsers() {
        ...
        user.list({search: search.value, user_type: user_type.value, orderBy: orderBy.value, page: page.value, paginate: true}).then((res) => {
