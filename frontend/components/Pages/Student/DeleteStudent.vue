            ...
            <PageLoading :pageLoading="pageLoading" :pageError="pageError" title="Student Not Found" :link="`/${accessType}/students`">
                <div class="flex justify-center">
                    <div class="text-center">
                        ...
                        <h4 class="mt-8 text-md font-extrabold tracking-tight text-slate-800">Are you sure you want to delete this student "{{student.getStudent ? student.getStudent.first_name : ''}}"?</h4>
                        <div class="flex justify-center mt-8">
                            <nuxt-link class="mx-2 group relative flex justify-center rounded-md border border-transparent bg-gray-600 py-3 px-4 text-sm font-medium text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2" :to="`/${accessType}/students`">Cancel</nuxt-link>
...
<script setup lang="ts">
   ...
   const loading = ref(false);
    ...
    function deleteStudent() {
        loading.value =  true;
        student.delete(props.id).then(res => {
            if (res.data.value) {
                success.value = 'Successfully deleted student.';
                setTimeout(() => {
                    router.push(`/${props.accessType}/students`);
                }, 2000);
            }
        });
    }
