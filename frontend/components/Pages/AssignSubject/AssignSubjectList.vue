        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-12 lg:grid-cols-12 gap-4">
                <div class="col-span-1 sm:col-span-1 md:col-span-3 lg:col-span-3 py-2">
                    <div class="overflow-hidden shadow sm:rounded-md border border-gray-100 rounded-lg">
                        <div class="bg-white py-5 p-6">
                            <form action="" method="post" @submit.prevent="searchAssignSubject">
                                <div class="grid grid-cols-1 gap-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Search</label>
                                        <input v-model="search" type="text" placeholder="Search AssignSubject"  class="mt-1 py-3 px-3 block w-full rounded-md border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    </div>
                                    <div>
                ...
                <div class="col-span-1 sm:col-span-1 md:col-span-9 lg:col-span-9">
                    <TableList 
                        v-slot="{row}" 
                        :loading="assignSubject_loading" 
                        :bodyData="assignSubject.getAssignSubjectData" 
                        :columns="columns" 
                        @selectedPage="pageSelected">
                        <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-semibold text-gray-700 sm:pl-6">{{row.subject.name}}</td>
                            ...
                            <Menu as="div" class="relative ml-3">
                                ...
                                <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                                    <MenuItems class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow ring-1 ring-black ring-opacity-5 focus:outline-none">
                                        <MenuItem>
                                            <nuxt-link :to="`/${accessType}/assign-subjects/schedules/${row.id}`" class="block px-4 py-2 text-sm text-gray-500">
                                                Schedules
                                            </nuxt-link>
                                        </MenuItem>
                                        <MenuItem>
                                            <nuxt-link :to="`/${accessType}/assign-subjects/edit/${row.id}`" class="block px-4 py-2 text-sm text-gray-500">
                                                Edit Assign Subject
                                            </nuxt-link>
                                        </MenuItem>
                                        <MenuItem>
                                            <nuxt-link :to="`/${accessType}/assign-subjects/delete/${row.id}`" class="block px-4 py-2 text-sm text-gray-500">
...
<script setup lang="ts">
    ...
    const assignSubject_loading = ref(false);
    const assignSubject = useAssignSubjectStore();
    const columns = ['Subject', 'Grade Level', 'Actions']
    ...
    function searchAssignSubject() {
        search_loading.value = true;
        fetchAssignSubjects();
    }

    function fetchAssignSubjects() {
        assignSubject_loading.value = true;
        assignSubject.list({search: search.value, orderBy: orderBy.value, page: page.value, paginate: true, school_id: null}).then((res) => {
            assignSubject_loading.value = false;
