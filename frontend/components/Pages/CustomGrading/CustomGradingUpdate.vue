<template>
    <NuxtLayout :name="accessType">
        ...
        <AlertErrorSuccess  
            ...
            <PageLoading :pageLoading="pageLoading" :pageError="pageError" title="CustomGrading Not Found" :link="`/${accessType}/customGradings`">
                <form action="" method="post" @submit.prevent="updateCustomGrading">
...
<script setup lang="ts">
    ...
    import { useCustomGradingStore, CustomGrading } from '~~/stores/customGrading';
    ...
    function updateCustomGrading() {

        loading.value = true;

        customGrading.update(props.id, models.value).then(res => {
            errors.value = [];
            success.value = '';
            loading.value = false;
            ...
            if (res.error.value) {
                ...
                success.value = 'Successfully saved custom grading';
            }
        });
    }
        ...
        await nextTick(async () => {
            ...
            customGrading.show(props.id).then(res => {
                ...
                if (data && Object.keys(data).length > 0) {
                    ...
                    data.custom_grading_grade_levels?.map(item => models.value.grade_level_ids.push({
                        text: item.grade_level ? item.grade_level?.name: '',
