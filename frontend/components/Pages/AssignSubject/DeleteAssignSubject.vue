...
<script setup lang="ts">
    ...
    const header = {
        ...
        breadcrumbs: [
            ...
            {name: 'Delete Assign Subject', link: '#'}
        ]
    };
   ...
   const loading = ref(false);
    ...
    function deleteAssignSubject() {
        loading.value =  true;
        assignSubject.delete(props.id).then(res => {
            if (res.data.value) {
                success.value = 'Successfully deleted assign subject.';
                setTimeout(() => {
                    router.push(`/${props.accessType}/assign-subjects`);
                }, 2000);
            }
        });
    }
